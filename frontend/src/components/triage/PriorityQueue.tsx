...
import { TriangleAlert, ShieldAlert, MapPin, Clock } from 'lucide-react';
...
export const PriorityQueue: React.FC<PriorityQueueProps> = ({
  ...
  const [queueTab, setQueueTab] = useState<'defects' | 'incidents'>(defaultTab);
    ...
    <div className="flex-1 flex flex-col min-h-0 select-none bg-white dark:bg-slate-900">
      ...
      <div className="flex items-center border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
        <button
          ...
          className={`flex-1 py-2 px-3 text-xs font-semibold flex items-center justify-center gap-1.5 transition border-b-2 ${
            queueTab === 'defects'
        ...
        >
          ...
          <span>Hazards ({clusters.length})</span>
        </button>
        <button
        ...
        >
          ...
          <span>{user.role === 'rto_officer' ? `Compliance (${incidents.length})` : `Safety (${incidents.length})`}</span>
        </button>
      </div>
